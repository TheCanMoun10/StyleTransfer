...
def transform_cxcywh(boxes, height, width):
    ...
    boxes[:, :] = np.minimum(0.999, np.maximum(0.001, boxes[:, :]))
...
def cdet_preprocess(images, boxes, pad=31, mean=[0.45, 0.45, 0.45]):
    ...
    dw = (inp_width - width) / 2
    dh = (inp_height - height) / 2
    ...
    for box in boxes:
        ...
        box[:, [1, 3]] += dh
    imgs = [
        cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=mean)
        for img in images
    ]

    return imgs, boxes, dw, dh

def cdet_augmentation(images, boxes, jitter=0.1, mean=[0.45, 0.45, 0.45]):
    height, width, _ = images[0].shape
    dw = int(width * jitter)
    dh = int(height * jitter)
    if height > width:
        pleft, pright = np.random.randint(-dw, 0, size=2)
        ptop, pbot = np.random.randint(0, dh, size=2)
    else:
        pleft, pright = np.random.randint(0, dw, size=2)
        ptop, pbot = np.random.randint(-dh, 0, size=2)
    ...
    imgs = [
        cv2.copyMakeBorder(img, max(0, -ptop), max(0, -pbot), max(0, -pleft), max(0, -pright), cv2.BORDER_CONSTANT, value=mean)
        for img in images
    ]
    imgs = [
        img[max(0, ptop): img.shape[0] - max(0, pbot), max(0, pleft): img.shape[1] - max(0, pright)]
        for img in imgs
    ]
    assert imgs[0].shape[0] == sheight
    assert imgs[0].shape[1] == swidth

    for box in boxes:
        box[:, [0, 2]] /= width
        box[:, [1, 3]] /= height
        box[:, 0] = np.minimum(0.999, np.maximum(0, box[:, 0] / sx - dx)) * swidth
        box[:, 1] = np.minimum(0.999, np.maximum(0, box[:, 1] / sy - dy)) * sheight
        box[:, 2] = np.minimum(0.999, np.maximum(0, box[:, 2] / sx - dx)) * swidth
        box[:, 3] = np.minimum(0.999, np.maximum(0, box[:, 3] / sy - dy)) * sheight

    return imgs, boxes


def box_augmentation(boxes):
    # zoom in or out the boxe a tiny
    pleft, pright = np.random.randint(-10, 10, size=2)
    ptop, pbot = np.random.randint(-10, 10, size=2)
...
def longer_scale(imgs, boxes, size, mean=[0.45, 0.45, 0.45]):
    """resize imgs longer side to desired size, pad shorter side with mean
    """
    ...
    if width < height:
        ...
        new_height = int(math.floor((float(height) / width) * size))
        boxes = [
            proposal * float(new_width) / width for proposal in boxes
        ]
        ratio = float(new_width) / width

    dw = (size - new_width) / 2
    dh = (size - new_height) / 2
    top, bottom = round(dh - 0.1), round(dh + 0.1)
    left, right = round(dw - 0.1), round(dw + 0.1)
    for box in boxes:
        box[:, [0, 2]] += dw
    ...
    imgs = [
        cv2.resize(img, (new_width, new_height), interpolation=cv2.INTER_LINEAR).astype(np.float32)
        for img in imgs
    ]
    imgs = [
        cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=mean)
    ...
    assert imgs[0].shape[0] == size and imgs[0].shape[1] == size

    return imgs, boxes, dw, dh, ratio
...
def resize_boxes(size, boxes, height, width):
    for box in boxes:
        box[:, [0, 2]] *= float(size) / width
        box[:, [1, 3]] *= float(size) / height
